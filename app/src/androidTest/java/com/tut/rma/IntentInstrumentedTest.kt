...
class IntentInstrumentedTest {
    ...
    fun useAppContext() {
        ...
        Assert.assertEquals("com.tut.rma", appContext.packageName)
    }
    @Test
    fun testDetailActivityInstantiation(){
        val pokreniDetalje: Intent = Intent(ApplicationProvider.getApplicationContext(),MainActivity2::class.java)
   ...
   onView(withId(R.id.movie_genre)).check(matches(withText("crime")))
        ...
        onView(withId(R.id.movie_poster)).check(matches(withImage(R.drawable.c)))
    }
    @Test
    fun testLinksIntent(){
        Intents.init()
